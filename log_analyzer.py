...
def open_log(config, file_name):
    file_name_vs_dir = replace_path(f'{config["LOG_DIR"]}/{file_name}')
    if file_name.endswith('.gz'):
        curr_open = gzip.open(file_name_vs_dir, 'rt', encoding='cp1251')
    else:
        curr_open = open(file_name_vs_dir, 'r', encoding='utf-8')
    with curr_open as f:
        file = f.read().split('\n')
...
def main(args):
    ...
    try:
        ...
        if max_file_name.startswith('nginx-access-ui'):
            file_data = open_log(config, max_file_name)
        else:
        ...
        logger1.error('skrypt was interrupted by keybord')
        quit(1)
